...
function placeBet(GameId gameId, uint256 betMask) public payable {
    ...
    //rollModulo => count of possible cases
    uint256 rollModulo = getRollModulo(gameId);
    //rollUnder => calculate the result need to be under this real num 
...
function settleBet(address gambler) public {
    ...
    require (bet.amount != 0,"zero amount bet!");

    // Check that the bet is neither too early nor too late.
    require (block.number > bet.placeBlockNumber + BLOCK_DELAY,"FUCK TOO FAST!");
    require (block.number <= bet.placeBlockNumber + BET_EXPIRATION_BLOCKS,"BLOCK_DELAY!");
...
function getRollUnderForT(uint256 rollModulo, uint256 betMask) pure public returns (uint256) {
    uint256 rollUnder = 0;
